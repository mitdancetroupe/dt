...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('prodWeek_prefsheet', (
        ...
        db.send_create_signal('prodWeek', ['PrefSheet'])

        # Adding unique constraint on 'PrefSheet', fields ['user', 'show']
        db.create_unique('prodWeek_prefsheet', ['user_id', 'show_id'])

        # Adding unique constraint on 'PrefSheet', fields ['audition_number', 'show']
        db.create_unique('prodWeek_prefsheet', ['audition_number', 'show_id'])

        # Adding model 'Pref'
        db.create_table('prodWeek_pref', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('prefsheet', self.gf('django.db.models.fields.related.ForeignKey')(related_name='prefs', to=orm['prodWeek.PrefSheet'])),
        ...
        db.send_create_signal('prodWeek', ['Pref'])

        # Adding unique constraint on 'Pref', fields ['prefsheet', 'dance']
        db.create_unique('prodWeek_pref', ['prefsheet_id', 'dance_id'])

        # Adding unique constraint on 'Pref', fields ['prefsheet', 'pref']
        db.create_unique('prodWeek_pref', ['prefsheet_id', 'pref'])


    def backwards(self, orm):
        # Removing unique constraint on 'Pref', fields ['prefsheet', 'pref']
        db.delete_unique('prodWeek_pref', ['prefsheet_id', 'pref'])

        # Removing unique constraint on 'Pref', fields ['prefsheet', 'dance']
        db.delete_unique('prodWeek_pref', ['prefsheet_id', 'dance_id'])

        # Removing unique constraint on 'PrefSheet', fields ['audition_number', 'show']
        db.delete_unique('prodWeek_prefsheet', ['audition_number', 'show_id'])

        # Removing unique constraint on 'PrefSheet', fields ['user', 'show']
        db.delete_unique('prodWeek_prefsheet', ['user_id', 'show_id'])

        # Deleting model 'PrefSheet'
        db.delete_table('prodWeek_prefsheet')

        # Deleting model 'Pref'
        db.delete_table('prodWeek_pref')
