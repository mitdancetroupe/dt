...
def prefsheet(request, show_slug):
    ...
    if not show.prefsheets_open:
        return render(request, 'prodWeek/closed.html', {'show': show})
    try:
    ...
    return render(request, 'prodWeek/prefsheet.html',
                              {'show': show,
...
def thanks(request, show_slug):
    return render(request, 'prodWeek/thanks.html',
                              context_instance=RequestContext(request))

@permission_required('prodWeek.can_list')
def select_dance_for_availability(request, show_slug):
    ...
    return render(request, 'prodWeek/availability_selection.html', {'dances':dances, 'show_slug':show_slug})
...
def availability(request, show_slug, dance_id):
    ...
    return render(request, 'prodWeek/availability.html',  context)
'''
/prodWeek/showslug/something
'''
...
def select_dance(request, show_slug):
    show = Show.objects.get(slug=show_slug)
    dances = Dance.objects.filter(show=show)
    return render(request, 'prodWeek/dance_selection.html', {'dances':dances, 'show_slug':show_slug})

@permission_required('prodWeek.can_list')
def selection(request, show_slug, dance_id):
    return render(request, 'prodWeek/selection.html', {'slug':show_slug, 'dance_id':dance_id})
...
def dancesheets(request, show_slug):
    ...
    return render(request, 'prodWeek/dancesheets.html',
                              {'dances': dances,
                               'show': show})
...
def prefsheets(request, show_slug):
    ...
    prefsheets = PrefSheet.objects.filter(show=show).extra(
            select={'first_pref': 'SELECT dance_id FROM prodWeek_pref WHERE pref=1 AND prefsheet_id=prodWeek_prefsheet.id'},
            order_by=['first_pref'])
    return render(request, 'prodWeek/prefsheets.html',
...
def assignments(request, show_slug):
    ...
    prefsheets = PrefSheet.objects.filter(show=show).order_by('user__last_name',
                                                              'user__first_name')
    return render(request, 'prodWeek/assignments.html',
                              {'prefsheets': prefsheets,
                               'show': show})
...
def csv(request, show_slug):
    ...
    filename = '%s-prodWeek.csv' % show_slug
